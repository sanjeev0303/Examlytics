...
type ExamService interface {
	...
	SubmitExam(ctx context.Context, clerkID string, req dto.SubmitExamRequest) (*dto.ExamResultResponse, error)
...
// GetExams retrieves all exams
func (s *ExamServiceImpl) GetExams(ctx context.Context) ([]*domain.Exam, error) {
	return s.examRepo.FindAllExams(ctx)
}
...
// SubmitExam calculates score and updates session
func (s *ExamServiceImpl) SubmitExam(ctx context.Context, clerkID string, req dto.SubmitExamRequest) (*dto.ExamResultResponse, error) {
	user, err := s.userRepo.FindByClerkID(ctx, clerkID)
	if err != nil {
		return nil, err
	}
