...
import { Loader2, CheckCircle2, XCircle, Clock, RotateCcw, BrainCircuit, Target, Zap } from "lucide-react";
...
const QuestionReviewList = memo(({ questions, answers }: { questions: Question[], answers: Record<string, unknown>[] }) => {
    return (
        <div className="space-y-4">
            {(questions || []).map((q: Question, i: number) => {
                const userAnswerObj = (answers || []).find((a: Record<string, unknown>) => a.questionId === q.id);
                const userAnswer = userAnswerObj?.answer as string | undefined || userAnswerObj?.selectedOption as string | undefined;
                const timeSpent = userAnswerObj?.timeSpent as number | undefined; // Extract time spent
